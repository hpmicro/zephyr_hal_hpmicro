...
function(sdk_src)
    foreach(file ${ARGN})
        ...
        if("${HPM_SDK_TARGET_SCOPE}" STREQUAL "private")
            target_sources(${HPM_SDK_PROJECT_NAME} PRIVATE ${path})
        elseif("${HPM_SDK_TARGET_SCOPE}" STREQUAL "public")
            target_sources(${HPM_SDK_PROJECT_NAME} PUBLIC ${path})
        else()
            message(FATAL_ERROR "ERROR:HPM_SDK_TARGET_SCOPE need to be defined")
        endif()
    endforeach()
endfunction()
...
function(sdk_sys_inc)
    foreach(inc ${ARGN})
        ...
        target_include_directories(${HPM_SDK_INTERFACE_NAME} SYSTEM INTERFACE ${path})
    endforeach()
endfunction()

function(sdk_get_compile_options opts)
    get_target_property(all_opts ${HPM_SDK_INTERFACE_NAME} INTERFACE_COMPILE_OPTIONS)
...
function(sdk_linker_global_symbols)
    foreach(sym ${ARGN})
        target_link_libraries(${HPM_SDK_INTERFACE_NAME} INTERFACE "-Xlinker --defsym=${sym}")
        get_property(link_symbols TARGET ${HPM_SDK_INTERFACE_NAME} PROPERTY INTERFACE_LINK_SYMBOLS)
        list(APPEND link_symbols "${sym}")
        set_target_properties(${HPM_SDK_INTERFACE_NAME} PROPERTIES INTERFACE_LINK_SYMBOLS "${link_symbols}")
    endforeach()
endfunction()

function(sdk_compile_options)
    foreach(opt ${ARGN})
        target_compile_options(${HPM_SDK_INTERFACE_NAME} INTERFACE ${opt})
    endforeach()
endfunction()

function(sdk_compile_definitions)
    foreach(def ${ARGN})
        target_compile_definitions(${HPM_SDK_INTERFACE_NAME} INTERFACE ${def})
    endforeach()
endfunction()

function(sdk_link_libraries)
    foreach(lib ${ARGN})
        target_link_libraries(${HPM_SDK_INTERFACE_NAME} INTERFACE ${lib})
    endforeach()
endfunction()

function(sdk_ld_options)
    foreach(opt ${ARGN})
        target_link_libraries(${HPM_SDK_INTERFACE_NAME} INTERFACE ${opt})
    endforeach()
endfunction()
...
function(sdk_clear_compile_options)
    set_target_properties(${HPM_SDK_INTERFACE_NAME} PROPERTIES INTERFACE_COMPILE_OPTIONS "")
endfunction()

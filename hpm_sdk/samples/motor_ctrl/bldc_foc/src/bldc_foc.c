...
void isr_adc(void)
{
    ...
    status = hal_adc_get_status_flags(&hal_adc_u);

    if ((status & BOARD_BLDC_ADC_TRIG_FLAG) != 0) {
        hal_adc_clear_status_flags(&hal_adc_u, BOARD_BLDC_ADC_TRIG_FLAG);
...
{
    adc_trig_config_t trig_cfg;

    trig_cfg.module = BOARD_BLDC_ADC_MODULE;

#if BOARD_BLDC_ADC_MODULE == ADCX_MODULE_ADC12
    trig_cfg.config.adc12.trig_ch   = trig_ch;
    trig_cfg.config.adc12.trig_len  = BOARD_BLDC_ADC_PREEMPT_TRIG_LEN;
    trig_cfg.config.adc12.adc_ch[0] = BOARD_BLDC_ADC_CH_W;
    trig_cfg.config.adc12.inten[0] = inten;

    trig_cfg.adc_base.adc12 = BOARD_BLDC_ADC_W_BASE;
#else
    trig_cfg.config.adc16.trig_ch   = trig_ch;
    trig_cfg.config.adc16.trig_len  = BOARD_BLDC_ADC_PREEMPT_TRIG_LEN;
    trig_cfg.config.adc16.adc_ch[0] = BOARD_BLDC_ADC_CH_W;
    trig_cfg.config.adc16.inten[0] = inten;

    trig_cfg.adc_base.adc16 = BOARD_BLDC_ADC_W_BASE;
#endif
    hal_adc_set_preempt_config(&trig_cfg);
#if BOARD_BLDC_ADC_MODULE == ADCX_MODULE_ADC12
    trig_cfg.config.adc12.adc_ch[0] = BOARD_BLDC_ADC_CH_V;
    trig_cfg.adc_base.adc12 = BOARD_BLDC_ADC_V_BASE;
#else
    trig_cfg.config.adc16.adc_ch[0] = BOARD_BLDC_ADC_CH_V;
    trig_cfg.adc_base.adc16 = BOARD_BLDC_ADC_V_BASE;
#endif
    hal_adc_set_preempt_config(&trig_cfg);
#if BOARD_BLDC_ADC_MODULE == ADCX_MODULE_ADC12
    trig_cfg.config.adc12.adc_ch[0] = BOARD_BLDC_ADC_CH_U;
    trig_cfg.adc_base.adc12 = BOARD_BLDC_ADC_U_BASE;
#else
    trig_cfg.config.adc16.adc_ch[0] = BOARD_BLDC_ADC_CH_U;
    trig_cfg.adc_base.adc16 = BOARD_BLDC_ADC_U_BASE;
#endif
    hal_adc_set_preempt_config(&trig_cfg);
}
...
void adc_init(void)
{
    ...
    hal_adc_init_default_config(&cfg);
#if BOARD_BLDC_ADC_MODULE == ADCX_MODULE_ADC12
    cfg.config.adc12.res            = adc12_res_12_bits;
    cfg.config.adc12.conv_mode      = adc12_conv_mode_preempt;
    ...
    hal_adc_init(&cfg);
#else
    cfg.config.adc16.conv_mode      = adc16_conv_mode_preempt;
    ...
    hal_adc_init_channel_default_config(&ch_cfg);
#if BOARD_BLDC_ADC_MODULE == ADCX_MODULE_ADC12
    ...
    adc16_set_preempt_queue_enable(BOARD_BLDC_ADC_U_BASE, BOARD_BLDC_ADC_TRG, true);
    adc16_set_preempt_queue_enable(BOARD_BLDC_ADC_V_BASE, BOARD_BLDC_ADC_TRG, true);
    adc16_set_preempt_queue_enable(BOARD_BLDC_ADC_W_BASE, BOARD_BLDC_ADC_TRG, true);
#endif

     /* Set DMA start address for preemption mode */
    hal_adc_init_pmt_dma(&hal_adc_u, core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t)adc_buff[0]));
    hal_adc_init_pmt_dma(&hal_adc_v, core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t)adc_buff[1]));
    hal_adc_init_pmt_dma(&hal_adc_w, core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t)adc_buff[2]));
}
/*
...
int main(void)
{
    ...
    hal_adc_enable_interrupts(&hal_adc_u, BOARD_BLDC_ADC_TRIG_FLAG);
    while(1)
    {
